...
function mutateRanksToAlphabetize(imported, alphabetizeOptions) {
  ...
  // const groupedRanksGroupedByPathStart = groupRanks.reduce((obj, rank) => {
  //   const groupPaths = groupedByRanks[rank];
  //   const byPathStart = groupByUpToAndFirstWord(groupPaths, alphabetizeOptions.caseInsensitive);
  ...
  //   return {
  //     ...obj,
  //     [rank]: map((arr) => arr.sort(comparator), byPathStart),
  //   };
  // }, {});

  // const suborderedRanks = groupRanks.sort().reduce((acc, groupRank) => {
  //   forEachOf((names, pathKey) => {
  //     names.forEach((importedItemName) => {
  //       acc[importedItemName] =
  //     });
  //   }, groupedRanksGroupedByPathStart[groupRank]);
  ...
  const alphabetizedRanks = groupRanks.sort().reduce((acc, groupRank, groupIndex) => {
    ...
    importNames.forEach((importedItemName, idx) => {
      if (idx < lastIndex) {
        ...
        if (importedItemName.startsWith('../')) {
          ...
          if (!nextImportName.startsWith(pathStart)) {
            breakLines.push(groupIndex + idx);
            linebreakAfter = true;
          }
        }
