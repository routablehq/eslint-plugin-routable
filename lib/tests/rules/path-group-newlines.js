...
function withoutAutofixOutput(t) {
  return { ...t, output: t.code };
}
...
ruleTester.run('order', rule, {
  valid: [
    ...
    // // ignoring unassigned values by default (as they're imported solely for side-effects)
    test({
      code: `
        import './foo';
        import 'fs';
    ...
    // // adding unknown import types (using a resolver alias, etc) to the groups
    test({
      code: `
        import fs from 'fs';
    ...
    // // adding multiple unknown import types to the groups
    test({
      code: `
        import fs from 'fs';
  ...
  invalid: [
    ...
    // builtin before external module
    test({
      code: `
           import async from 'async';
           import fs from 'fs';
